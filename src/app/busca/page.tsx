    ...
    <div className="max-w-full overflow-x-hidden px-4 py-10 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-8">
          ...
          {query && (
            <p className="mt-2 text-surface-50/60">
              {results.length}{" "}
        ...
        <div className="mb-8 flex flex-wrap gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-medium text-surface-50/60">
              Categoria:
            </span>
            <Link
        ...
        {results.length > 0 ? (
          ...
          <div className="rounded-xl border border-surface-800 bg-surface-900 px-6 py-16 text-center">
            ...
            <h2 className="mt-4 text-lg font-semibold text-surface-50">
              {query
                ? "Nenhuma fórmula encontrada"
                : "Digite algo para buscar"}
            </h2>
            <p className="mt-2 text-sm text-surface-50/60">
              {query
                ? "Tente usar termos diferentes ou remova os filtros."
                : "Busque por nome, conceito ou equação."}
            </p>
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              <span className="text-sm text-surface-50/40">
                Sugestões:
              </span>
              {["Newton", "Bhaskara", "Cinemática", "Energia"].map((term) => (
                <Link
                  ...
                  className="rounded-full border border-surface-700 bg-surface-800 px-3 py-1 text-sm text-surface-50 transition-colors hover:border-primary-500 hover:text-primary-400"
                >
                  {term}
                </Link>
              ))}
